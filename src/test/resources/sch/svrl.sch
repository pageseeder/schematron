...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" xml:lang="en">
  ...
  <sch:p>This schema can be used to validate SVRL documents, and provides examples
    of the use of abstract rules and abstract patterns.
  </sch:p>
  ...
  <sch:pattern>
    ...
    <sch:rule context="svrl:schematron-output">
      ...
      count(svrl:successful-report) = count(*)">
        <sch:name/>
        may only contain the following elements: text,
    ...
    <sch:rule context="svrl:diagnostic-reference">
      ...
      <sch:assert test="string-length(@diagnostic) &gt; 0">
        <sch:name/>
        should have a diagnostic attribute, giving the id of
        the diagnostic.
      </sch:assert>
    </sch:rule>
    <sch:rule context="svrl:ns-prefix-in-attribute-values">
      ...
      >A
        <sch:name/>
        comes before an active-pattern or another
    ...
    <sch:rule context="svrl:fired-rule">
      ...
      >A
        <sch:name/>
        comes after an active-pattern, an empty fired-rule,
        a failed-assert or a successful report.
      </sch:assert>
      <sch:assert test="string-length(@context) &gt; 0">The
        <sch:name/>
        element
    ...
    <!-- Catch-all rule-->
    <sch:rule context="*">
      <sch:report test="true()">An unknown
        <sch:name/>
